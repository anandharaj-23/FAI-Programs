...
def alpha_beta(node,depth,alpha,beta):
    ...
    if node.ismax:
        ...
        for child in node.children:
            value,path=alpha_beta(child,depth+1,alpha,beta)
            ...
            if beta<=alpha:
                print(f"Pruning at {child.name} with alpha {alpha} and beta {beta}")
                break
...
o.add_child(v4)
o.add_child(v5)
best_score, best_path = alpha_beta(root, 0,float('-inf'),float('inf'))
...
o.add_child(v4)
o.add_child(v5)
best_score_ab, best_path_ab = alpha_beta(root, 0, float('-inf'), float('inf'))
